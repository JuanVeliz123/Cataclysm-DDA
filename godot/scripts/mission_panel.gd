extends Control
## The missions screen ("Your missions") -- four tabs, a row list, a detail
## pane -- as a Godot Control.
##
## The game thread is blocked in mission_ui's run_in_godot while this is up,
## and that loop still owns every action: the tab, which row is selected, and
## closing. This panel replaces where the action comes from and nothing else.
##
## Unlike the other MENU-13 panels, switching tabs here changes which
## collection exists at all (active/completed/failed missions, or points of
## interest are different lists, not different views of one list) -- the
## panel does not need to know that; it just shows whatever `rows` the game
## thread published for the tab it asked for.
##
## See src/godot_mission_snapshot.h.
...
func _build() -> void:
	...
	_objective.add_theme_color_override("font_color", N.ACCENT)
	...
	# Rows on the left, the selected row's detail on the right: the same split
	# the ImGui screen draws, so a player who knows one knows the other.
	_split = HBoxContainer.new()
	_split.add_theme_constant_override("separation", N.SPACE_M)
	_split.size_flags_vertical = Control.SIZE_EXPAND_FILL
	col.add_child(_split)
	...
	confirm_btn.text = "Set as objective"
	confirm_btn.focus_mode = Control.FOCUS_NONE
	N.apply_button(confirm_btn)
	confirm_btn.pressed.connect(func() -> void: _act("CONFIRM"))
	button_row.add_child(confirm_btn)
	_delete_btn = Button.new()
	_delete_btn.text = "Delete point of interest"
	_delete_btn.focus_mode = Control.FOCUS_NONE
	N.apply_button(_delete_btn)
	_delete_btn.visible = false
	_delete_btn.pressed.connect(func() -> void: _act("DELETE_POINT_OF_INTEREST"))
	button_row.add_child(_delete_btn)

	col.add_child(N.micro_label("↑↓ select · ←→ tabs · Enter set objective · Esc close", N.NEUTRAL_700))

# --- update -------------------------------------------------------------------

func refresh() -> void:
	if _host == null or not _host.has_method("get_mission_state"):
		return
	if _detail == null:
		_build()
	var gen: int = int(_host.mission_generation())
	if gen == _generation:
		return
	_generation = gen

	var d: Dictionary = _host.get_mission_state()
	var title := str(d.get("title", ""))
	if title != "":
		_title.text = title.to_upper()
	_tab = int(d.get("tab", 0))
	_selected = int(d.get("selected", 0))
	_can_delete = bool(d.get("can_delete", false))
	_rebuild_tabs(d.get("tabs", []))

	var objective := str(d.get("current_objective", ""))
	_objective.visible = objective != ""
	_objective.text = objective

	var rows: Array = d.get("rows", [])
	var empty_text := str(d.get("empty_text", ""))
	_empty_label.visible = rows.is_empty()
	_empty_label.text = empty_text
	_split.visible = not rows.is_empty()
	if not rows.is_empty():
		_rebuild_rows(rows)
		_detail.text = TAGS.to_bbcode(str(d.get("detail", "")))
	_delete_btn.visible = _can_delete and not rows.is_empty()
...
func _act(action: String) -> void:
	if _host != null and _host.has_method("mission_action"):
		_host.mission_action(action)

func _select_tab(index: int) -> void:
	if index != _tab and _host != null and _host.has_method("mission_select_tab"):
		_host.mission_select_tab(index)

func _select_row(index: int) -> void:
	if index != _selected and _host != null and _host.has_method("mission_select_row"):
		_host.mission_select_row(index)
