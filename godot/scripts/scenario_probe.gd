...
func _probe_help() -> void:
	...
	await _probe_compare_item_menu()

## `compare_item_menu` (2026-09-01) -- the second-to-last screen on BACKLOG's
## "remaining ImGui screens" table. No selection state, no tabs: two
## independently-scrolled read-only panes plus an optional CONFIRM/QUIT bar.
## The real path is crafting, the inventory `compare` command, or a gunmod
## swap; debug_open_compare_item_menu() is the same shortcut
## debug_open_follower_rules() is for its own screen, adding two guaranteed
## items to the avatar's inventory instead of walking any of those paths (or
## depending on what an evac-shelter spawn happens to be carrying).
func _probe_compare_item_menu() -> void:
	if not host.has_method("get_compare_item_state"):
		_check("compare_item channel exists", false, true, "no binding (old library?)")
		return
	if not host.has_method("debug_open_compare_item_menu"):
		_check("compare_item debug hook exists", false, true, "no binding (old library?)")
		return
	host.debug_open_compare_item_menu()
	var opened := false
	var waited := 0.0
	var d: Dictionary = {}
	while waited < 10.0:
		await get_tree().create_timer(0.25).timeout
		waited += 0.25
		d = host.get_compare_item_state()
		if host.compare_item_active() or bool(d.get("active", false)):
			opened = true
			break
	_check("compare_item screen opened", opened, true, "after %.1fs" % waited)
	if not opened:
		return

	var first_body := str(d.get("first_body", ""))
	var second_body := str(d.get("second_body", ""))
	print("[scn] [compare_item] first='%s' (%d chars) second='%s' (%d chars)" % [
		str(d.get("first_name", "")), first_body.length(),
		str(d.get("second_name", "")), second_body.length()])
	_check("compare_item published both panes", first_body.length() > 0 and second_body.length() > 0,
		true, "first %d chars, second %d chars" % [first_body.length(), second_body.length()])

	host.compare_item_action("QUIT")
	var closed := false
	var shut := 0.0
	while shut < 6.0:
		await get_tree().create_timer(0.25).timeout
		shut += 0.25
		if not host.compare_item_active():
			closed = true
			break
	_check("compare_item screen closed", closed, true, "after QUIT")

	await _probe_end_screen()

## The end-of-game screen (2026-09-01) -- the last row on BACKLOG's "remaining
## ImGui screens" table; landing this and compare_item_menu unblocks MENU-9.
## No selection state, and simpler than every MENU-13/14 screen: one static
## body and one optional text field, one outcome (CONFIRM).
## debug_open_end_screen() calls draw_end_screen_ui(actually_dead = false)
## directly, so this never touches g->uquit or fires game_avatar_death.
func _probe_end_screen() -> void:
	if not host.has_method("get_end_screen_state"):
		_check("end_screen channel exists", false, true, "no binding (old library?)")
		return
	if not host.has_method("debug_open_end_screen"):
		_check("end_screen debug hook exists", false, true, "no binding (old library?)")
		return
	host.debug_open_end_screen()
	var opened := false
	var waited := 0.0
	var d: Dictionary = {}
	while waited < 10.0:
		await get_tree().create_timer(0.25).timeout
		waited += 0.25
		d = host.get_end_screen_state()
		if host.end_screen_active() or bool(d.get("active", false)):
			opened = true
			break
	_check("end_screen opened", opened, true, "after %.1fs" % waited)
	if not opened:
		return

	var body := str(d.get("body", ""))
	print("[scn] [end_screen] body_len=%d has_text_input=%s" % [body.length(),
		str(d.get("has_text_input", false))])
	_check("end_screen published body", body.length() > 0, true, "%d chars" % body.length())

	host.end_screen_confirm("scenario probe")
	var closed := false
	var shut := 0.0
	while shut < 6.0:
		await get_tree().create_timer(0.25).timeout
		shut += 0.25
		if not host.end_screen_active():
			closed = true
			break
	_check("end_screen closed", closed, true, "after CONFIRM")
