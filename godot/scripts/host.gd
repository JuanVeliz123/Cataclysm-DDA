...
var advanced_inv_panel: Control
var dialogue_panel: Control
var surroundings_panel: Control
## MENU-13's screens, one channel each.
var mission_panel: Control
var faction_panel: Control
var martialarts_panel: Control
var scores_panel: Control
var medical_panel: Control
var study_zone_panel: Control
var follower_rules_panel: Control
var distraction_panel: Control
var auto_note_panel: Control
var color_manager_panel: Control
var safemode_panel: Control
var auto_pickup_panel: Control
var diary_panel: Control
var compare_item_panel: Control
var end_screen_panel: Control
...
func _update_advanced_inv_panel() -> void:
	if advanced_inv_panel == null or not cdda_host.has_method("advanced_inv_active"):
		return
	var want: bool = cdda_host.advanced_inv_active()
	if want:
		advanced_inv_panel.refresh()
	if want != advanced_inv_panel.visible:
		advanced_inv_panel.visible = want

func _advanced_inv_open() -> bool:
	return advanced_inv_panel != null and advanced_inv_panel.visible
...
func _update_menu13_panels() -> void:
	for spec in [["martialarts", martialarts_panel], ["scores", scores_panel],
			["medical", medical_panel], ["study_zone", study_zone_panel],
			["mission", mission_panel], ["faction", faction_panel],
			["diary", diary_panel], ["follower_rules", follower_rules_panel],
			["distraction", distraction_panel], ["auto_note", auto_note_panel],
			["color_manager", color_manager_panel], ["safemode", safemode_panel],
			["auto_pickup", auto_pickup_panel], ["compare_item", compare_item_panel],
			["end_screen", end_screen_panel]]:
...
func _menu13_open() -> bool:
	for panel in [martialarts_panel, scores_panel, medical_panel, study_zone_panel, mission_panel,
			faction_panel, diary_panel, follower_rules_panel, distraction_panel, auto_note_panel,
			color_manager_panel, safemode_panel, auto_pickup_panel, compare_item_panel,
			end_screen_panel]:
...
func _is_session_hotkey(_event: InputEvent) -> bool:
	# The Godot inventory, character and game-menu panels are not wired up yet:
	# i, @ and Escape belong to the game's own screens until they are.
	return false
...
func _handle_session_hotkey(event: InputEvent) -> bool:
	if (_popup_modal() or _textwin_open() or _uilist_open() or _options_open()
			...
			or _surroundings_open() or _menu13_open() or _advanced_inv_open()):
		return false
