...
done. MENU-14 is done too (`help` landed 2026-08-31, the last of its six
screens). **`end_screen` and `compare_item_menu` are now done too** (2026-09-02,
the last two rows of MENU-13's screen table).

**MENU-9 is blocked, not unblocked -- corrected 2026-09-12.** This doc
previously claimed that landing `end_screen`/`compare_item_menu` meant
"nothing left in a session opens a `catacurses::window` that a Godot takeover
does not already own," and that MENU-14's row claimed the same ("nothing draws
into `catacurses::window` in a session"). Both statements are false and were
never actually checked against the source tree. `grep -c
defined(GODOT)` against every file that still calls `catacurses::newwin(`
turns up dozens with **zero** Godot-specific code, none of them out-of-session:
`player_display.cpp` (the character sheet), `veh_interact.cpp`,
`armor_layers.cpp`, `bionics_ui.cpp`, `mutation_ui.cpp`, `zone_manager_ui.cpp`,
`proficiency_ui.cpp`, `bodygraph.cpp`, `panels.cpp`, `options.cpp`,
`debug_menu.cpp`, `mission_companion.cpp`, `faction_camp.cpp`,
`construction.cpp`, `computer_session.cpp`, `trade_ui.cpp`,
`smart_controller_ui.cpp`, `morale.cpp`, `iuse.cpp`, `ranged.cpp`,
`live_view.cpp`, `messages.cpp`, `iexamine.cpp`, `inventory_ui.cpp`,
`string_input_popup.cpp` (the *non*-imgui class), `main_menu.cpp` and
`worldfactory.cpp` (pre-session, but still needed to ever reach one) among
them -- see MENU-9's row below for the full picture. Every one of these is
only visible today because `godot_curses_backend.cpp` renders their cells into
`ViewSnapshot` and `terminal_view.gd` draws that grid as a generic fallback;
`look_around()`'s own `w_info` (`godot_look_snapshot.h`) documents this
dependency directly. Deleting the bridge before all of these have their own
Godot panel would make the main menu, world/character creation, the character
sheet, and dozens of other in-session screens unusable. **Do not start MENU-9
until this list is empty** -- migrating (or explicitly deciding not to
migrate) each of these screens is now the real remaining work of Part 2, not
a one-line deletion.

Part 3 (verification) is new and mostly open — read VER-1 before doing more
rendering work. Part 4 (the 3D backend) is a **proposal**, not a queue: read
ADR-006 before taking anything out of it, and note that its own third
milestone is allowed to cancel the rest.
...
| ~~`compare_item_menu`~~ | — | opens from other screens | **done (2026-09-02)** -- no selection state at all, just two independently-scrolled read-only panes (the same `format_item_info()` + `remove_color_tags()` conversion `ui_iteminfo.cpp` already uses for a single pane, called twice) plus an optional CONFIRM/QUIT bar. Kept as its own `CompareSnapshot` rather than extending `TextWinSnapshot` to a two-pane mode, to avoid touching that class's well-exercised single-pane path (item info, extended description, help). `debug_open_compare_item_menu()` compares two freestanding items constructed on the spot rather than pulled from the avatar's own inventory -- the first attempt used `Character::i_add()`, which defaults to `allow_wield = true` and put the first item straight into `weapon` on an empty-handed fixture instead of `inv`, so `const_slice()` never saw it; freestanding items sidestep loadout and pocket space entirely, since the screen only ever reads its two `const item &` arguments. Verified via `scenario_probe.gd`: both panes publish (2270 and 1253 chars), QUIT closes it | `godot_compare_snapshot.*`, `compare_item_panel.gd`, `game_inventory.cpp`, `godot_game_commands.*` |
...
| ~~`end_screen`~~ | **none** | ImGui nav | **done (2026-09-02)** -- checked for the same escape hatch `npctalk_rules` had and did not need it: CONFIRM here is not addressing a selection at all, it is the *only* outcome (there is nothing to pick), so there was never a nav-focus problem to solve. The art-plus-overlay selection logic (`select_end_screen()`, picking the highest-priority matching `end_screen` by its `condition`) was factored out of `draw_controls()` so both the legacy ImGui draw and the new Godot path pick the same screen; the overlay `added_info` entries are spliced into their row at their column by padding then appending, a simplified re-presentation of the ImGui path's `ImGui::SameLine` cursor placement (which can overwrite characters already drawn there -- nothing in this game's own data ever expects that). The "last words" field is bundled into the CONFIRM request rather than synced continuously, since nothing reads it before then. `debug_open_end_screen()` calls `draw_end_screen_ui(actually_dead = false)` directly, the same code path the real death handler runs minus the `game_avatar_death` event, so a probe can open and close it without ending a session. Verified via `scenario_probe.gd`: body publishes (798 chars), CONFIRM closes it | `godot_end_screen_snapshot.*`, `end_screen_panel.gd`, `end_screen.cpp`, `godot_game_commands.*` |
...
- **`npctalk_rules` and `end_screen` have no selection variable at all** — the
  ...
  addressing a keypress does; no model needed. **`end_screen` was checked for
  the same escape hatch (done 2026-09-02) and did not need one either** -- its
  own CONFIRM has no selection to read in the first place; see its row below.
...
done. This table is now fully done: `end_screen` and `compare_item_menu`
landed 2026-09-02.

| ~~MENU-14~~ | M | The six `catacurses::window` settings screens — safe mode, auto pickup, auto notes, distractions, colors, help. **All six done** (2026-08-31): `distraction_manager`, `auto_note`, `color_manager`, `safemode`, `auto_pickup`, `help`. **These were why the cell buffer still existed.** "One channel probably serves all six" (the original note here) undersold `help`, which turned out to need none of its own -- see its row below. `color_loader.cpp` in the file list above never existed, the colors screen is `color_manager::show_gui()` in `color.cpp` | those files | **Done for these six** -- but see the corrected Status note above: many *other* screens still draw straight into `catacurses::window` with no Godot branch at all, so this row's old claim that nothing does "in a session" was wrong |
...
| MENU-9 | **Was S, is now much larger -- blocked** | ~~Delete the overlay: `terminal_view.gd`, the ImTui blit, the ImGui cell layer, `USE_CURSES_UI_OVERLAY`~~ **Corrected 2026-09-12: not a deletion task yet.** `godot_curses_backend.cpp` is not a debug overlay -- it *is* the Godot build's entire catacurses rendering backend (`catacurses::init_interface()`, `curses_drawwindow()`, `refresh_display()`). Auditing every file that still calls `catacurses::newwin(` for a `#if defined(GODOT)` branch (`grep -c "defined(GODOT)"`) found these with **none**, meaning they are only visible today via the `terminal_view.gd` fallback and would go dark the moment it is deleted: `player_display.cpp`, `veh_interact.cpp`, `armor_layers.cpp`, `bionics_ui.cpp`, `mutation_ui.cpp`, `zone_manager_ui.cpp`, `proficiency_ui.cpp`, `bodygraph.cpp`, `panels.cpp`, `options.cpp`, `debug_menu.cpp`, `mission_companion.cpp`, `faction_camp.cpp`, `construction.cpp`, `computer_session.cpp`, `trade_ui.cpp`, `smart_controller_ui.cpp`, `morale.cpp`, `iuse.cpp`, `ranged.cpp`, `live_view.cpp`, `messages.cpp`, `iexamine.cpp`, `inventory_ui.cpp`, `string_input_popup.cpp`, `main_menu.cpp`, `worldfactory.cpp`. (`advanced_inv.cpp` and `uilist.cpp` have partial GODOT branches already and need checking case-by-case, not assumed done.) `look_around()`'s `w_info` also leans on this bridge on purpose (see `godot_look_snapshot.h`) and needs its own decision, not just a panel. The real MENU-9 is: migrate (or knowingly decide to leave on the legacy path) every screen in that list, *then* delete the bridge -- treat each as its own MENU-13/14-sized task, not one ticket | `terminal_view.gd`, `godot_view_snapshot.*`, `godot_curses_backend.cpp`, plus every file listed above | Not started -- the cell buffer cannot go yet |
