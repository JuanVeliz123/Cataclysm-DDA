...
- **First run on macOS (2026-08-29) found one real timing fix and one real bug,
  after two wrong theories along the way -- read this before trusting the next
  "stuck legacy screen" diagnosis.** Every previous verification claim in this
  file was made on the Linux box above; a run on a Mac (arm64, `arch -arm64`,
  otherwise identical command) had every screen fail to open by keypress: not
  just the newly-added diary probe, but surroundings, martial arts, medical,
  the overmap, crafting, options, keybindings and dialogue.
  1. **First theory, wrong:** the log's `world render target ... is not the
     drawable area in device pixels` (a ~20x mismatch) looked like a macOS
     display-scale bug. It is `headless_probe.gd`'s own throwaway probe-world
     sizing math, unrelated to the live session -- a red herring that cost a
     round trip.
  2. **Real fix:** the game thread takes 13-20s+ after "session active" before
     it reaches its first `get_input_event()` call at all -- confirmed with a
     temporary heartbeat print in the wait loop, zero output for the first
     ~15-20s, then steady. `session active` is a Godot-side flag set early;
     whatever the game thread does next (first-turn NPC processing is the
     leading suspect, given nearby allies/hostiles) is not covered by it.
     Fixed in `scenario_probe.gd`'s `_wait_until_responsive()`, which polls
     `get_map_generation()` while pressing `.` (pause) for up to 30s before
     any interactive stage runs, and reports how long it actually took.
  3. **Second theory, wrong:** a second, harder stall recurred right after the
     probe's first teleport into fresh terrain (`scenario_teleport_omt` to
     `"field"`). `legacy_ui_active()` went true and never cleared; raising the
     watchdog's patience to 30s (20 Escapes) made no difference. Instrumenting
     `cataimgui::window`'s constructor (temporary `printf`, removed after)
     named the actual window: **`'SURROUNDINGS'`** -- the probe's own scripted
     `V` press opening MENU-12's screen, not an ambient monologue. The
     "soliloquy window" theory (written up as `BACKLOG.md`'s SOLILOQUY-1) was
     wrong and has been retracted there.
  4. **Real fix:** `surroundings_menu::publish_to_godot()` looped
     `for (t = items; t != num_tabs; ++t)`, but the file's own
     `operator++(surroundings_menu_tab_enum&)` is written for cyclic
     NEXT_TAB/PREV_TAB navigation and wraps `num_tabs` back to `0` -- so the
     loop's exit value never occurs. An infinite loop, on the game thread,
     every time this screen published, on any platform, since it was written.
     That is also the real explanation for MENU-12's nine failed observation
     attempts (eight before this one, logged below) -- never a timing or
     attend-contract problem. Fixed by iterating the underlying ints instead
     of the cyclic operator (`surroundings_menu.cpp`, 2026-08-29).
  5. Confirmed by re-running `scenario_probe.gd` end to end: surroundings,
     martial arts, medical, diary and scores all open, publish, round-trip an
     action and close, each in ~0.2s. One small, separate, unfixed issue
     remains: surroundings' own NEXT_TAB round trip does not change tabs (see
     `BACKLOG.md` MENU-12), the same "tab-lift" class of bug `mission_ui` and
     `faction_ui` already had to solve for their own tab state.
  The lesson worth keeping: a `cataimgui::window` id is one `printf` away and
  answers "what is this screen" directly. Both wrong theories here were spent
  reasoning about symptoms (blank cells, timing, backgrounds) that a five-line
  instrumented rebuild would have settled immediately.
