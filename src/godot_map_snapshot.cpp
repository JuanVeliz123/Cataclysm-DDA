...
void MapSnapshot::update_from_game()
{
    ...
    {
        // `center` is the *view* center -- u.pos_bub() + u.view_offset -- which
        // during look_around() is the look cursor, not the avatar. Using it here
        // made the avatar's mesh ride along with the cursor instead of staying
        // put, indistinguishable from actually walking there. view_offset is
        // zero outside of look/peek, so this was invisible until look-around's
        // view panning actually worked under Godot.
        const tripoint_bub_ms avatar_pos = u.pos_bub( here );
