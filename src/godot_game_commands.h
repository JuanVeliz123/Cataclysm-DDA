...
/**
 * Open `compare_item_menu` on two freestanding items constructed on the
 * spot, not tied to the avatar's own inventory -- the screen only ever reads
 * its two `const item &` arguments, so this sidesteps a fresh evac-shelter
 * fixture's starting loadout and pocket space entirely.
 *
 * The real path is opened from crafting, the inventory `compare` command, or
 * a gunmod attach/remove -- each needing a fixture of its own to reach. This
 * is the same shortcut @ref request_debug_open_follower_rules is for its own
 * screen: skip the path, not the screen.
 *
 * @return always empty; queued unconditionally.
 */
std::string request_debug_open_compare_item_menu();

/**
 * Open the end-of-game screen without ending the game.
 *
 * Calls `end_screen_data::draw_end_screen_ui( actually_dead = false )`
 * directly -- the exact code path the real death handler runs, minus the
 * `game_avatar_death` event -- so a probe can open and close it repeatedly.
 *
 * @return always empty; queued unconditionally.
 */
