...
AutoPickupSnapshot g_auto_pickup_snapshot;
...
{
    return g_auto_pickup_snapshot;
}

bool AutoPickupSnapshot::active() const
{
...
uint64_t AutoPickupSnapshot::generation() const
{
    std::lock_guard<std::mutex> lock( mutex_ );
    return generation_;
}

godot::Dictionary AutoPickupSnapshot::copy_state() const
{
    const_cast<AutoPickupSnapshot *>( this )->note_attended();
    std::lock_guard<std::mutex> lock( mutex_ );
    godot::Dictionary d;
    ...
    d["auto_pickup_on"] = data_.auto_pickup_on;

    godot::Array titles;
    titles.resize( static_cast<int64_t>( data_.tab_titles.size() ) );
    for( size_t i = 0; i < data_.tab_titles.size(); ++i ) {
        titles[static_cast<int64_t>( i )] = gs( data_.tab_titles[i] );
    }
    d["tab_titles"] = titles;
...
void AutoPickupSnapshot::request_action( const std::string &action )
{
    std::lock_guard<std::mutex> lock( mutex_ );
    pending_actions_.push_back( action );
}

void AutoPickupSnapshot::request_tab( const int tab )
{
    pending_tab_.store( tab, std::memory_order_relaxed );
}

void AutoPickupSnapshot::request_confirm( const int encoded )
{
    pending_confirm_.store( encoded, std::memory_order_relaxed );
}

void AutoPickupSnapshot::request_remove( const int row )
{
    pending_remove_.store( row, std::memory_order_relaxed );
}

void AutoPickupSnapshot::request_copy( const int row )
{
    pending_copy_.store( row, std::memory_order_relaxed );
}

void AutoPickupSnapshot::request_swap( const int row )
{
    pending_swap_.store( row, std::memory_order_relaxed );
}

void AutoPickupSnapshot::request_enable( const int row )
{
    pending_enable_.store( row, std::memory_order_relaxed );
}

void AutoPickupSnapshot::request_disable( const int row )
{
    pending_disable_.store( row, std::memory_order_relaxed );
}

void AutoPickupSnapshot::request_move_up( const int row )
{
    pending_move_up_.store( row, std::memory_order_relaxed );
}

void AutoPickupSnapshot::request_move_down( const int row )
{
    pending_move_down_.store( row, std::memory_order_relaxed );
}

void AutoPickupSnapshot::request_test( const int row )
{
    pending_test_.store( row, std::memory_order_relaxed );
}

void AutoPickupSnapshot::note_attended()
{
    attended_.store( true, std::memory_order_relaxed );
}

bool AutoPickupSnapshot::attended() const
{
    return attended_.load( std::memory_order_relaxed );
}

void AutoPickupSnapshot::publish( const data &d )
{
    std::lock_guard<std::mutex> lock( mutex_ );
    active_ = true;
    data_ = d;
...
void AutoPickupSnapshot::set_suspended( const bool suspended )
{
    suspended_.store( suspended, std::memory_order_relaxed );
}

void AutoPickupSnapshot::clear()
{
    {
        std::lock_guard<std::mutex> lock( mutex_ );
        active_ = false;
...
std::string AutoPickupSnapshot::next_action( int &row, int &col )
{
    row = -1;
    col = -1;
