...
crafting_ui_impl::build_godot_detail() const
{
    ...
    if( avail.can_craft_recipe && recp.deduped_requirements().is_too_complex() ) {
    ...
    const bool npc_cant = avail.crafter.is_npc() && !recp.npc_can_craft( npc_reason )
                          && !avail.inv_override;
    if( !avail.can_craft_recipe && avail.apparently_craftable && !npc_cant ) {
    ...
    const temp_crafting_inventory &inv = avail.inv_override ? *avail.inv_override : crafter->crafting_inventory();
...
void crafting_ui_impl::publish_to_godot()
{
    ...
    for( size_t i = 0; i < current.size(); ++i ) {
        ...
        if( i < available.size() ) {
            ...
            r.craftable = a.can_craft_recipe;
