...
std::string request_debug_open_follower_rules()
{
    post_game_command( []() {
        ...
        for( npc &guy : g->all_npcs() ) {
            ...
            const int dist = rl_dist( u.pos_bub(), guy.pos_bub() );
        ...
        if( nearest == nullptr ) {
            add_msg( m_debug, "no NPC to open follower rules for" );
            return;
        }
        // Same reasoning as request_menu_action: this opens a blocking screen
        // from inside the input wait, and is only safe because the Godot
        // takeover it runs into (or falls back from) reliably dismisses it.
        follower_rules_ui new_ui;
        new_ui.draw_follower_rules_ui( nearest );
    } );
    return std::string();
}

std::string request_debug_open_compare_item_menu()
{
    post_game_command( []() {
        if( !g ) {
            return;
        }
        // Two freestanding items, not tied to the avatar's own inventory --
        // compare_item_menu only ever reads its two `const item &` arguments
        // (info(), tname()...), so this does not need them carried, wielded,
        // or dropped anywhere. That sidesteps a fresh evac-shelter avatar's
        // starting loadout and pocket space entirely.
        const item first( itype_id( "knife_combat" ) );
        const item second( itype_id( "rock" ) );
        // Same reasoning as request_debug_open_follower_rules: opens a
        // blocking screen from inside the input wait, safe only because the
        // Godot takeover it runs into (or falls back from) always dismisses
        // it.
        game_menus::inv::compare_item_menu( first, second ).show();
    } );
    return std::string();
}

std::string request_debug_open_end_screen()
{
    post_game_command( []() {
        // actually_dead = false: exercises the exact same screen the real
        // death path opens, without killing the avatar or firing the
        // game_avatar_death event, so a probe can call this repeatedly.
        end_screen_data{}.draw_end_screen_ui( /*actually_dead=*/false );
    } );
    return std::string();
}
...
{
    const map_extra_id id( map_extra );
    if( !id.is_valid() ) {
        return "No such map extra.";
    }
    post_game_command( [id]() {
        if( !g ) {
            return;
        }
        get_auto_notes_settings().set_discovered( id );
        scenario_done( "discover_map_extra", true, id.str() );
    } );
    return std::string();
}
